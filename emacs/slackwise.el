...
;; (advice-add 'org-roam-node-slug :before
;;             (lambda (node)
;;               (setq debug-on-error t)
;;               (setf (org-roam-node-title node)
;;                     (s-replace "\"" "" (s-replace "'" "" (org-roam-node-title node))))
;;               (pp node)
;;               '(node)))
